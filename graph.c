...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	//check to see if they are the same vertex
	...
	while (!dequeIsEmpty(stack))
	{
		...
		//check current vertex against destination
		if (vertex == destination)
		{
			//clear deques
		...
		//if not the destination, add neighbors
		for (int i = 0; i < vertex->numNeighbors; i++)
		{
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	//check to see if source and destination are the same vertex
	if (source == destination)
	{
		return 1; 
	}
