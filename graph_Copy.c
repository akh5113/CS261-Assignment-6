...
{
    // FIXME: Implement;
	//clear before starting the search
	clearVisited(graph);

	//create container for reachable values
	Deque * reachable = dequeNew();

	//create a stack for neighbors
	Deque * stack = dequeNew();

	//push start vertext into reachable container
	dequePushFront(stack, source);

	//while the stack is not empty
	while (!dequeIsEmpty(stack))
	{
		//remove back entry from stack and assign to temp vertex
		Vertex * vertex = dequeBack(stack);

		dequePushBack(reachable, vertex);
		//remove from stack
		dequePopBack(reachable);
		//check to see if temp is already in reachable
		//while (vertex->isVisited == 0)
		//{
			//add temp vertex to reachable 
			//dequePushBack(reachable, vertex);
			//add neighbors of temp to the stack inf not already in reachable
			for (int i = 0; i < vertex->numNeighbors; i++)
			{
				Vertex* temp = vertex->neighbors[i];
				if (temp->isVisited == 0)
				{
					dequePushBack(stack, temp);
				}
			}
			//change isVisited to = 1
			vertex->isVisited = 1;

			/*
			//check to see if destination is in reachable
			//Type temp = dequeBack(reachable);
			//if (temp == destination);
			if (vertex == destination);
			{
				return 1;
...
int bfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	//push start vertext into reachable container
	dequePushFront(queue, source);

	//while the stack is not empty
	while (!dequeIsEmpty(queue))
	{
		//remove front entry from queue and assign to temp vertex
		Vertex * vertex = dequeFront(queue);
		//check to see if temp is already in reachable
		//while (vertex->isVisited == 0)
		//{
			//add temp vertex to reachable 
			dequePushBack(reachable, vertex);
			//remove vertext from queue
			dequePopFront(reachable);
			//add neighbors of temp to the stack inf not already in reachable
			for (int i = 0; i < vertex->numNeighbors; i++)
			{
				Vertex* temp = vertex->neighbors[i];
